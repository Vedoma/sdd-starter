...
| --- | --- | --- | --- |
| C1 | **The specification is the single source of truth.** No code is written without a corresponding spec entry; no spec entry is left silently un-implemented or un-rejected. | `partial` | `spec-lint`: the documents `sdd.config.yml` resolves to must exist, keep the starter's canonical section headings (level, wording and case — sections may be added and renumbered), and carry no unfilled scaffold placeholders, and each backlog task, change task and PR must carry a Spec Reference that **resolves where it can** — a `§N` must be a numbered heading of `docs/spec/technical-spec.md` (or, next to a `CHANGE-NNNN`, a section that change's `spec-delta.md` touches), and a `CHANGE-NNNN` must be a change directory, active or archived. The PR body must also keep every heading of `.github/PULL_REQUEST_TEMPLATE.md` at its level and replace the template's own placeholders — the structure is read from the template at runtime, and a bot-authored PR or a body with a reasoned `spec-lint: skip-pr-template` line skips both PR-body checks, with a note. **Still presence-only**: a reference into another document (`data-model.md §3`) or in prose, which `spec-lint` notes but cannot resolve. **Review-only**: whether the referenced section covers the work, and whether what the filled template says is true |
...
| C3 | **Accepted specs are never edited silently.** Changes go through the amendment process. | `partial` | `spec-lint` checks **mode consistency**: `sdd.config.yml` must declare `process.mode`; under `greenfield` no `docs/changes/CHANGE-*` directory may exist; under `sustain` `docs/spec/technical-spec.md` must be `Accepted`, its newest Revision History version must equal `SPEC_VERSION.md`'s Current Version, and a PR that edits `docs/spec/**` must also touch a change directory; every change directory is named `CHANGE-NNNN` and has a row in `docs/changes/README.md` whose Status matches its proposal. It also checks the **mechanics of delivery**: a `Delivered` change must be archived, an archived change must have been delivered and be cited by a [`SPEC_VERSION.md`](./SPEC_VERSION.md) Changelog row, and a PR that archives a change must also change `SPEC_VERSION.md` and the living spec. **Review-only**: whether the folded spec edit actually matches the delta, the `<!-- AMENDED -->` markers, and amendments under `greenfield`, where an accepted spec can still be edited with none of this bookkeeping (version drift there only warns) |
| C4 | **ADRs are immutable once accepted.** Supersede with a new ADR; never edit or delete a settled one. | `partial` | `spec-lint` checks the **registry and acceptance**: every ADR file has a row in `docs/adr/README.md` whose Status matches its front-matter `status`, every row has its file, and a PR that changes an ADR still `proposed` fails — acceptance happens on the open PR via `/adr accept`, which also updates the row. **Immutability itself is unchecked**: editing the body of an accepted ADR passes, and so does deleting an ADR together with its registry row. The `adr-status` workflow manages status transitions, but does not defend the rest of the file |
...
| C8 | **Every change is reviewable and reversible.** Atomic PRs; one ADR per PR; changes archived, never deleted. | `partial` | Branch protection per [`docs/repo-setup.md`](./docs/repo-setup.md) — a **settings-level** guarantee, not code. The one-ADR-per-PR rule is enforced only inside the `/adr` command. Archive hygiene is checked only for changes: a `Delivered` change must be moved to `docs/changes/archive/`, and a registered change whose directory is gone fails (deleting the directory together with its registry row passes). Moving delivered backlog tasks to `docs/plan/archive/` is not checked |
