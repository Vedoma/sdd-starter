...
| --- | --- | --- | --- |
| C1 | **The specification is the single source of truth.** No code is written without a corresponding spec entry; no spec entry is left silently un-implemented or un-rejected. | `partial` | `spec-lint`: the documents `sdd.config.yml` resolves to must exist and carry no unfilled scaffold placeholders, and each backlog task, change task and PR must carry a Spec Reference that **resolves where it can** — a `§N` must be a numbered heading of `docs/spec/technical-spec.md` (or, next to a `CHANGE-NNNN`, a section that change's `spec-delta.md` touches), and a `CHANGE-NNNN` must be a change directory, active or archived. **Still presence-only**: a reference into another document (`data-model.md §3`) or in prose, which `spec-lint` notes but cannot resolve. Whether the referenced section actually covers the work is review |
