...
| --- | --- | --- | --- |
| C1 | **The specification is the single source of truth.** No code is written without a corresponding spec entry; no spec entry is left silently un-implemented or un-rejected. | `partial` | `spec-lint`: the documents `sdd.config.yml` resolves to must exist; once the spec is filled in they must keep the starter's canonical section headings (level, wording and case, a subsection under its section — sections may be added and renumbered), and they, `SPEC_VERSION.md` and active change directories must not keep the scaffold's own placeholder markers (a fixed list — `[Product Name]`, `[Name]`, a `YYYY-MM-DD` date and a few more; other template hints such as `[Primary Persona Name]` are review-only); each backlog task, change task and PR must carry a Spec Reference that **resolves where it can** — a `§N` (both ends of a range `§N-M`) must be a numbered heading of `docs/spec/technical-spec.md` (or, next to a `CHANGE-NNNN` or in that change's own tasks, a section that change's `spec-delta.md` touches), and a change id must be spelled `CHANGE-NNNN` and be a change directory, active or archived. The PR body must also keep every heading of `.github/PULL_REQUEST_TEMPLATE.md` at its level and replace the template's own placeholders — the structure is read from the template at runtime and re-checked when the description is edited. Exemptions, each with a note: a dependency or release bot on the workflow's `PR_EXEMPT_BOTS` allowlist skips both PR-body checks (any other bot, a coding agent included, does not), and a `spec-lint: skip-pr-template` line whose reason cites a `#N` skips the structure check only. **Still presence-only**: a `§` that follows another document's name (`data-model.md §3`) and a value naming neither form (`N/A`, prose), which `spec-lint` notes but cannot resolve. **Review-only**: whether the referenced section covers the work, and whether what the filled template says is true |
